...
//   useEffect(() => {
//     // This effect runs when user.username changes.
//     // We use a ref to prevent it from running on the initial component mount.
//     if (isInitialMount.current) {
//       isInitialMount.current = false;
//       return;
//     }

//     // The username has been updated, so run your function
//     if (user?.username) {
//       handleUsernameUpdate(user.username);
//     }

//   }, [user?.username]); // The dependency array is key!

//   const handleUsernameUpdate = async (username, id) => {
...
//       body: JSON.stringify({ username, handle: username }),
//     });
...
//   } catch (err) {
//     toast.error("❌ Failed to sync username");
//     console.error(err);
//   }
// };
...
// handleUsernameUpdate('new-username-123');
...
//       if (user?.username) {
//         const res = await fetch("/api/findPage", {
//           method: "POST",
//           headers: { "Content-Type": "application/json" },
...
//   }, []);
  ...
  return (
    <nav className="bg-white w-[90vw] mx-auto fixed top-10 right-[5vw] rounded-full p-4 flex justify-between z-10 ">
